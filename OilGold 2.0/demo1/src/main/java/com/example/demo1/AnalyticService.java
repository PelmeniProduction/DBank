...
public class AnalyticService {
    ...
    public static Date isDifferenceLessThan(Date start,Date end,int days){
        Date dReturn=start;
        Calendar cal = Calendar.getInstance();
        long lStart=start.getTime();
        long lEnd=end.getTime();
        long lDiff=lEnd-lStart;
        if(lDiff>24*60*60*1000*days){
            dReturn=DataCrawler.subtractFromDate(days,start);
        }
        return dReturn;
    }
    public static void reloadDataFromDate(String category,Connection connection, Statement stmt, Date start,Date end)throws Exception{//начало и конец отрезка который выбрал юзер
        ...
        int iDiffForSmallDate=25;
        Date start1=isDifferenceLessThan(start,end,iDiffForSmallDate);

        cachedData= DataBase.getListFromDate(cat,connection,stmt,start1,end);//получить из бд список объектов TimeSeriesData
        //вычислять...
        ...
        //удалим если добавляли
            while(cachedAnalyzedData.get(0).getDate().getTime()<start.getTime()){
                cachedAnalyzedData.remove(0);
                cachedData.remove(0);
            }
    ...
    private static ArrayList<Double> arGetEMA(ArrayList<TimeSeriesData> cachedData){
        ...
        for (int i = 0; i < n; i++) {
            if(i==0)
                ...
                arEMA.add(i,cachedData.get(i).getPrice());
            else
                //arEMA.add(i,(cachedData.get(i).getPrice()*alpha)+(arEMA.get(i-1)*(1-alpha)));
                arEMA.add(i,arEMA.get(i-1)+alpha*(cachedData.get(i).getPrice()-arEMA.get(i-1)));
        }
        return arEMA;
    }
